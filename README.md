...
- **月蝕綺譚 14体**（CryptoNinja 外伝「月蝕綺譚 -Luna Occulta-」の御霊）: 於兎・シャオラン・オロチ・エマ・ネム・餡音・カルマ・柴・アトザ・アウン・トバリ・サスラ・石舟斎・栞。3D モデルは公式配布のモデルを使っています（下の「月蝕綺譚について」）
- キャラもステージも **three.js の 3D**（トゥーン調・輪郭線・リアルタイムの影）。CNP 9体のモデルは手続き生成で、色・体型・顔の特徴は[素材屋CNP](https://sozaiya.cryptoninja-partners.xyz/)の公式イラストを見て寄せています（イラスト自体は使っていません）
...
CryptoNinja 外伝「月蝕綺譚 -Luna Occulta-」（Studio VIBE）の御霊 14 体も、CNP と同じ一覧から選べます。里・五行・忍術の名前・口調は、月蝕綺譚の公式の設定資料 MCP（`kitan-lore`）と公式サイトの正典シートの公開情報から取りました。説明文・勝利画面の一言・性能は本作オリジナルです（一言は公式の口調に合わせて書いたもので、公式のセリフ例はそのまま使っていません）。

| 御霊 | 里・五行 | 得物（通常ワザ） | 必殺ワザ（名前は公式の忍術から） |
| --- | --- | --- | --- |
| 於兎 | 甲賀・土 | 兎拳 | うさぎどーん！／うさぎどっかーん！ |
| シャオラン | 甲賀・土 | 太極拳 | 口寄せ・リーリー／肩ならべ双掌打 |
| オロチ | 風魔・水 | 叢雲（刀） | 水遁・白露／鎌首／叢雲抜き放ち |
| エマ | 甲賀・金 | 一角槍 | 夢路・星こぼし／蝕夢・角むすび |
| ネム | 甲賀・水 | 絵筆 | 動植綵絵／旭日鳳凰図 |
| 餡音 | 伊賀・木 | 団子串 | 団子忍術・串千本 |
| カルマ | 風魔・火 | 鎖鎌 | 業独楽／お迎え鎖／領域・罪業 |
| 柴 | 雑賀・水 | 柴拳 | 水遁・渦車／体術・瀬走り |
| アトザ | 風魔・金 | 鬼の金棒 | 逢魔刻・影踏み／鬼鉄・地均し |
| アウン | 風魔・土 | 仁王拳 | 白絹・振り初め／阿吽・結び解き |
| トバリ | 根の国・水 | 夜の翼 | 帳・宵闇ノ幕／夢路・浅キ眠リ |
| サスラ | 根の国・金 | 神事の大鎌 | 邪気眼・封印緩メ／神事・一ノ鎌 |
| 石舟斎 | 伊賀・火 | 無刀 | 無刀取り |
| 栞 | 道しるべ | 式書 | 式書・御霊呼び |

- **3D モデル**: 月蝕綺譚の二次創作「[3Dの間](https://vibe.co.jp/luna-occulta/fanworks/models)」で配布されている公式モデル（ゲーム版 GLB）を、[二次創作ガイドライン](https://vibe.co.jp/luna-occulta/fanworks)に沿ってゲームに組み込んでいます。**モデルそのままの再配布（モデル集としての転載）はできないので、GLB はリポジトリに入れていません。** `npm run sync:kitan`（`dev` / `build` / `e2e` の前にも自動で実行）で素材蔵から取得し、台帳の sha256 と照合して `src/assets/kitan/models/`（git 管理外）に置きます。一覧は `src/assets/kitan/manifest.json`。取れなかったキャラは仮の形で表示されます。
- 公式モデルの骨は 6 本（脚は 1 本）なので、元の割り当てと頂点の位置から、共通骨格の肩・肘・股・膝へ割り当て直して動かしています（`src/render3d/characters/kitan.ts`）。オロチ（とぐろの蛇の体）と栞（床まである裾）は脚を分けません。
- 選択画面・勝利画面の月蝕綺譚の御霊は、公式のデザインのトンマナ「宵闇に金」（宵闇藍の地・金泥の細い線・月白の本文・明朝の見出し・縁なしの吹き出し）に合わせています。
- CNP と名前がかぶるオロチ・エマは別のキャラ（ID は `k_orochi`・`k_emma`）。
...
- 二次創作のルールは [CNP 二次創作ガイドライン](https://www.cryptoninja-partners.xyz/fanart-guideline.html)・[CNP AI 生成ガイドライン](https://www.cryptoninja-partners.xyz/ai-guideline.html)・[CryptoNinja 利用ガイドライン](https://www.ninja-dao.com/guidelines)で定められています。CNP の商用利用は、NFT を持っていない個人・法人はライセンス審査・契約が必要です（[ライセンスガイドライン](https://www.cryptoninja-partners.xyz/guidelines.html)）。このリポジトリを元に公開・配布・商用利用する前に必ず確認してください。本作での確認結果は [docs/GUIDELINES.md](docs/GUIDELINES.md)。
- CNP 9体の 3D モデルは本作で手続き生成したものです。公式イラストはリポジトリに含めず、ゲーム中でも使用していません。見た目は素材屋CNP の公式イラストを AI に読み込ませて寄せました。[素材屋CNP の AI 生成ガイドライン](https://sozaiya.cryptoninja-partners.xyz/guidelines)（AI に読み込ませるのは合計 10 点まで）に従い、使ったのは 1 体 1 点・合計 9 点だけで、画像は作業後に削除しています。同ガイドラインのとおり、キャラクターのデザインの権利は元のイラストの制作者と CNP にあります。
- 月蝕綺譚の御霊の 3D モデルは月蝕綺譚の公式配布モデルです（二次創作ガイドラインの範囲で組み込み）。#月蝕綺譚
...
- ゲームのプログラム・ステージ・エフェクト・効果音・BGM はすべて本作オリジナル（手続き生成）です。
