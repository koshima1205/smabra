...
関連 IP の「月蝕綺譚」は、公式サイトで[二次創作ガイドライン](https://vibe.co.jp/luna-occulta/fanworks)と、設定資料の MCP（`kitan-lore`・原作用の `cn-lore`。認証不要）の[つなぎ方](https://vibe.co.jp/luna-occulta/fanworks/mcp)を公開しています。開発に使ったクラウド環境には `kitan-lore` のコネクタが無かったため、今回は CNP で実装しました。
キャラの定義（`src/data/cnp.ts`）・性能の組み立て（`src/game/roster.ts`）・3D モデル（`src/render3d/characters/`）が分かれているので、同じ仕組みで月蝕綺譚のキャラを追加できます。
...
- キャラクターの 3D モデル・ステージは本作オリジナルの手続き生成です。公式イラストはリポジトリに含めず、ゲーム中でも使用していません。
