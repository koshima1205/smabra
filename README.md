...
- キャラもステージも **three.js の 3D**（トゥーン調・輪郭線・リアルタイムの影）。キャラのモデルはすべて手続き生成で、公式イラストは使っていません
...
関連 IP の「月蝕綺譚」にも公式の MCP（`kitan-lore`、会員向け）があります。開発に使ったクラウド環境には接続が無く、素材のホスト（vibe.co.jp など）への通信も許可されていなかったため、今回は CNP で実装しました。
