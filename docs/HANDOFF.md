...
- 「あそびかた」「クレジット」は、遊ぶ人が読んで分かる言葉で書く（MCP のツール名・同期日時・コマンドなどは画面に出さず README へ）。操作はタブ（キーボード／パッド／スマホ）で、1つの操作に代表のキーを1つだけ見せる
- キャラ選択は情報をしぼる（ユーザーの希望、2026-09-26）: キャラの札は顔と名前だけ、プレイヤーの枠は顔・名前・人/CPU/なし・CPU のつよさだけ。説明文・パートナー・タグ・ステータスの棒・ワザ名・操作のヒントの行は出さない。「いざ、勝負」は画面の真ん中に重ねず、見出しの行の右端のボタン（全員が決めると出る）
- 時間の初期値は 3 分（2026-09-26。保存データに版番号 `v` を入れ、前の版で「なし」のままだった人も 3 分になる。自分で選んだ時間はそのまま）
